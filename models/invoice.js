...
const renderDocumentOfHash = (req, res) => {
  ...
  const empresa = global.electronic_document_companies.find(
    (e) => e.value === ruc
  );
  const tipo = global.electronic_document_types.find((e) => e.value === puesto);
  const pathFilePDF = path.join(
    process.cwd(),
    "storage_invoices",
    year,
    empresa?.name || "sin-ruc",
    "pdf",
    tipo?.directory || "sin-tipo",
    `${filename}.pdf`
  );
  const pathFileXML = path.join(
    process.cwd(),
    "storage_invoices",
    year,
    empresa?.name || "sin-ruc",
  ...
  (response.base64PDF = foundPDF
    ? fs.readFileSync(pathFilePDF, { encoding: "base64" })
    : ""),
    (response.baseXML = foundXML
      ? fs.readFileSync(pathFileXML, { encoding: "utf8" })
      : ""),
    res.send(response);
};
...
const getInvoiceSAP = async (req, res) => {
  ...
  try {
    const data = await api.getInvoiceSAP(
      empresa,
      folio,
      numero,
      tipo,
...
const processFiles = (files) => {
  global.electronic_document_is_loading = true;
  async.eachSeries(
    files,
    (file, next) => {
...
const writeLogError = async (file, message) => {
  try {
    ...
    await fsPromises.rename(
      path.join(process.cwd(), "storage_invoices_listener", file),
...
const handlerObtenerTokenSunat = (empresa, callback) => {
  const SUNAT_ID =
    empresa === "datacont"
      ? process.env.API_DATACONT_SUNAT_CLIENT_ID
      : process.env.API_REPRODATA_SUNAT_CLIENT_ID;
  const SUNAT_SECRET =
    empresa === "datacont"
      ? process.env.API_DATACONT_SUNAT_CLIENT_SECRET
      : process.env.API_REPRODATA_SUNAT_CLIENT_SECRET;
  ...
  request.post(URL, opciones, function (err, response, body) {
    if (err) callback(err);
      ...
      if (response.statusCode === 200) callback(null, body);
        ...
        callback(
          `Existe un error al momento de obtener el, token de sunat - codigo error ${response.statusCode}`
        );
    }
  });
};
...
const getStatusInvoiceSunat = async (req, res) => {
  ...
  var objEmpresa = global.electronic_document_companies.find(
    (e) => e.name === empresa
  );
  ...
  handlerObtenerTokenSunat(empresa, function (err, acceso) {
    ...
    request.post(url, opciones, (err, resp, body) => {
      ...
      const objResponse = {
        ...global.electronic_document_response_sunat_type.find(
          (e) => e.id === +bodyParse.data.estadoCp
        ),
