...
.relative(v-if="!empresa")
  section.w-full.h-screen.bg-slate-700.h-screen.opacity-90(style="background-image: url('lima-san-isidro.jpg');background-repeat: no-repat;background-size: cover;background-blend-mode: multiply")
  section.absolute.top-0.w-full.h-screen.items-center.justify-center.flex-col.flex.px-4
    ...
    .p-6.w-full.max-w-sm.bg-white.rounded-lg.border.shadow-md(class='sm:p-6' )
      ...
      ul.my-8.space-y-4.transition-all.duration-300.ease-in-out
        li(v-for="(e, index) in empresas" :key="index")
          a.flex.items-center.p-3.text-base.text-gray-900.bg-gray-100.rounded-lg.group.cursor-pointer(@click="seleccionarEmpresa(e)" class='hover:bg-gray-100 hover:shadow  ')
            ...
            font-awesome-icon.text-xl.inline-flex.items-center.justify-center(icon='fa fa-check-circle' :class="empresaSelect === e.name ? 'text-green-400' : 'text-gray-200'" )
      div
...
.flex(v-else)
  section.text-white.flex-1.w-full.h-screen.bg-datacont-600.opacity-90.relative.flex.items-center.justify-center.hidden(class="lg:block md:block"  style="background-image: url('lima-san-isidro.jpg');background-repeat: no-repat;background-size: cover;background-blend-mode: multiply")
    ...
    .flex.mx-10.h-full.justify-center.items-center
      //- .flex-1
      .flex-1
        h1.text-2xl.font-extrabold.tracking-tight.leading-none.text-white(class='md:text-4xl lg:text-5xl') Comprobantes electronicos
        .flex.items-center.my-7.opacity-80
    ...
    .w-full(
      ...
      .p-6.space-y-4(class="md:space-y-6 sm:p-8",)
        .flex-1.flex.items-center.justify-center
          a
            img.w-60.h-20(src="/public/logo_completo_datacont.svg", alt="datacont" v-if="empresa === 'datacont'")
            img.w-60.h-20(src="/public/logo_completo_reprodata.svg", alt="reprodata" v-if="empresa === 'reprodata'")
        .space-y-4(class="md:space-y-6")
          .flex
            .flex-1
              ...
              select.bg-gray-100.border.border-gray-200.text-gray-500.rounded-lg.block.w-full.placeholder-gray-300.font-bold#tipo(
                v-model="tipo",
                name="tipo",
          ...
          .flex
            .flex-1.mr-4
              ...
              input.bg-gray-100.border.border-gray-200.text-gray-500.rounded-lg.block.w-full.placeholder-gray-300.font-bold#folio(
                v-model="folio",
                type="text",
                name="folio",
                class="sm:text-xs focus:ring-datacont-500 focus:border-datacont-600 p-2.5",
            ...
            .flex-1
              ...
              input.bg-gray-100.border.border-gray-200.text-gray-500.rounded-lg.block.w-full.placeholder-gray-300.font-bold#numero(
                v-model="numero",
                type="number",
                name="numero",
                class="sm:text-xs focus:ring-datacont-500 focus:border-datacont-600 p-2.5   ",
          ...
          .flex
            .flex-1.mr-4
              ...
              input.bg-gray-100.border.border-gray-200.text-gray-500.rounded-lg.block.w-full.placeholder-gray-300.font-bold#fecha(
                v-model="fecha",
                type="date",
                name="fecha",
                class="sm:text-xs focus:ring-datacont-500 focus:border-datacont-600 p-2.5 ",
            ...
            .flex-1
              ...
              input.bg-gray-100.border.border-gray-200.text-gray-500.rounded-lg.block.w-full.placeholder-gray-300.font-bold#monto(
                v-model="monto",
                type="text",
                name="monto",
                class="sm:text-xs focus:ring-datacont-500 focus:border-datacont-600 p-2.5   ",
          ...
          button.w-full.text-white.bg-gradient-to-r.from-datacont-400.via-datacont-500.to-datacont-600.shadow-lg.font-medium.rounded-lg.text-xs.px-5.text-center.mr-2.mb-2(
            class="hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-datacont-300  shadow-datacont-500/50  py-4"
        ...
        if (data.length) {
          if (data[0]?.PDF || data[0]?.XML) {
            ...
            notificacion(
              "error",
              "Error",
              "El documento no cuenta con representacion grafica (pdf, xml)"
            );
        } else
          notificacion(
            "error",
            "Error",
            "Los datos ingresados no coinciden con ningun comprobante electronico"
          );
      } catch (error) {
        notificacion(
          "error",
    ...
    const continuarConLaEmpresaSeleccionado = () => {
      if (!empresaSelect.value) {
        notificacion(
          "error",
          "Error",
          "Seleccione alguna empresa para continuar"
        );
      } else {
        router.push(`/search-invoice?company=${empresaSelect.value}`);
        empresa.value = empresaSelect.value;
